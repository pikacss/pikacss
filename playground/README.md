...
- **Dependency cache** (`useSnapshotCache.ts` + `useWebContainer.ts`): to avoid the ~90s in-browser `npm install` on repeat visits, once the dev server is ready the app calls `webcontainer.export('.', { format: 'binary' })`, gzips it, and stores it in IndexedDB keyed by `<template>@<hash of package.json>`. On the next visit it decompresses + `mount()`s that snapshot, `chmod -R +x node_modules` (mount drops the executable bit), and skips install — dev-ready in ~6–7s instead of ~90s. Falls back to a normal install when no cache exists or mounting fails.
  - This uses WebContainer's **own** export (its WASM-swapped rollup/esbuild), which is why it re-mounts correctly. A snapshot built from a host `npm install` does **not** work — see the note in the repo `AGENTS.md`.
  - The cache key is the template's `package.json` (which carries the build-time-rewritten `@pikacss` version), so a new release naturally invalidates stale snapshots.
