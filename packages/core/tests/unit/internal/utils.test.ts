...
describe('utils', () => {
	it('should warn', () => {
		...
		setWarnFn(warnFn)
		warn('hello')
		expect(warnFn).toHaveBeenCalledWith('hello')
	})
	...
	it('should correctly identify property values', () => {
		expect(isPropertyValue('red')).toBe(true)
		expect(isPropertyValue(10)).toBe(true)
		expect(isPropertyValue(null)).toBe(true)
		expect(isPropertyValue(undefined)).toBe(true)
		expect(isPropertyValue(['red', 'blue'])).toBe(false)
		expect(isPropertyValue({ color: 'red' })).toBe(false)
		expect(isPropertyValue(['prop', ['val1', 'val2']])).toBe(true)
		expect(isPropertyValue(['prop', ['val1', { obj: 'val2' }]] as any)).toBe(false)
	})
	...
	it('should append autocomplete selectors', () => {
		...
		expect(config.autocomplete.selectors).toEqual(new Set(['a', 'b']))
	})
	...
	it('should append autocomplete style item strings', () => {
		...
		expect(config.autocomplete.styleItemStrings).toEqual(new Set(['a', 'b']))
	})
	...
	it('should append autocomplete extra properties', () => {
		...
		expect(config.autocomplete.extraProperties).toEqual(new Set(['a', 'b']))
	})
	...
	it('should append autocomplete extra css properties', () => {
		...
		expect(config.autocomplete.extraCssProperties).toEqual(new Set(['a', 'b']))
	})
	...
	it('should append autocomplete property values', () => {
		...
		expect(config.autocomplete.properties.get('color')).toEqual(['red', 'blue'])
	})
	...
	it('should append autocomplete css property values', () => {
		...
		expect(config.autocomplete.cssProperties.get('color')).toEqual(['red', 'blue'])
	})
	...
	it('should render css style blocks', () => {
		...
		expect(renderCSSStyleBlocks(blocks, true)).toBe('.a {\n  color: red;\n}\n.b {\n  color: blue;\n}')
		expect(renderCSSStyleBlocks(blocks, false)).toBe('.a{color:red;}.b{color:blue;}')
	})
})
