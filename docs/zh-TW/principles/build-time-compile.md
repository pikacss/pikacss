...
插件使用 regex 掃描每個原始檔案，尋找所有 `pika()` 呼叫，包含格式變體（`pika.str()`、`pika.arr()`、`pika.inl()`）和預覽變體（`pikap()`）。它使用括號深度解析器來正確提取完整的函式呼叫，處理巢狀括號、字串、註解和樣板字面值。
...
`pika.inl()` 會產生無周圍引號或陣列語法的裸 class 名稱。TypeScript 將 `pika.inl()` 的回傳型別標註為 `void`，以對出編譯後的值不應被當作一般字串或陣列使用。此格式適用於需要將原始 class 名稱標號內嵌入运算的特定樣板或框架定制語法中：
