...
::: info 全域函式
`pika()` 是一個全域函式——你無需匯入它。建置插件會在原始碼轉換期間偵測並替換這些呼叫。產生的 `pika.gen.ts` 透過 `declare global` 提供 TypeScript 支援，而非透過模組匯出。
:::

### 階段二：引數求值

提取的引數字串會在建置時使用 `new Function(...)` 進行求值。例如，`pika({ color: 'red', fontSize: '16px' })` 的引數會被解析為 JavaScript 物件字面值並執行。在實務上，最安全的使用方式仍是維持 ESLint 規則所要求的 literal-only 子集合，因為這能讓建置流程保持可預測且避免副作用。
...
PikaCSS 支援兩種轉換後 `pika()` 呼叫的輸出格式。格式可透過 `transformedFormat` 插件選項全域設定，或使用方法變體強制指定每次呼叫的格式。
...
由於引數會在建置時使用 `new Function(...)` 求值，建議的使用方式是將它們維持在 literal-only 的子集合中，也就是 `@pikacss/eslint-config` 所強制的範圍。

**有效——建置時可求值的靜態值：**

<<< @/.examples/principles/build-valid-usage.ts

**避免——會讓建置流程更難推理的執行期值與動態表達式：**
