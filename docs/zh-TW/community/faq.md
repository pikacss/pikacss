...
PikaCSS 使用 base-52 編碼（`a`–`z`、`A`–`Z`）為每個唯一的屬性-值-選擇器組合指派一個短 class 名稱。預設情況下，引擎會先加上 `pk-` 前綴，因此第一個原子化樣式是 `pk-a`，第二個是 `pk-b`，依此類推。超過 52 個 class 後，名稱會變成 `pk-aa`、`pk-ba` 等。若想使用不帶前綴的 `a`、`b`、`c`，可在引擎設定中明確指定 `prefix: ''`。

## 為何 PikaCSS 建議 `pika(...)` 使用靜態引數？

PikaCSS 完全在建置時期運作。整合轉換透過正規表達式找到 `pika(...)` 呼叫，並使用 `new Function('return [...]')` 評估其引數表達式。在實務上，建議將引數維持在 literal-only 的子集合中，讓建置流程保持可預測且避免副作用；這也是 `@pikacss/eslint-config` 所強制的範圍。
...
不需要。`pika()` 是一個**全域函式**——你可以直接在任何原始檔案中使用它，無需匯入。建置插件會在原始碼轉換過程中找到符合條件的 `pika()` 呼叫，並在建置時期將它們替換為產生的 class 名稱。`pika.gen.ts` 檔案透過 `declare global` 提供 TypeScript 自動補齊，但它不是可以匯入的模組。你不應該撰寫 `import { pika } from '...'`。
