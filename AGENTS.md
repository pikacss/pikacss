...
- Repeat visits skip the ~90s in-browser `npm install` via a **client-side dependency cache** (`useSnapshotCache.ts`): after `server-ready` the app `export('.', { format: 'binary' })`s the container, gzips it, and stores it in IndexedDB keyed by `<template>@<package.json hash>`; the next visit `mount()`s it, runs `chmod -R +x node_modules` (mount drops the exec bit), and skips install. This works **only** because it re-mounts WebContainer's *own* export (its WASM-swapped rollup/esbuild). Do **not** replace it with a host-built static snapshot: WebContainer disables native `.node` addon loading and resolves WASM builds of native-dependent packages (rollup, esbuild) only via an *in-container* `npm install` through StackBlitz's proxy registry, so a host `node_modules` ships incompatible binaries and fails at startup (`ERR_DLOPEN_DISABLED` for rollup, esbuild "service was stopped"). A CI-generated static snapshot would need `export()` run inside a headless WebContainer, not a host install.
...
- Marketing/example copy that shows a literal `pika()` call must wrap it in a JS string (`{'pika()'}` in JSX, `{{ 'pika()' }}` in Vue) — the transform's regex scanner (`stripLiteral` + `\bpika\(`) matches bare `pika()` in JSX/template **text** and rewrites it to `''`.
