...
- Static baseline snapshots are generated by `scripts/gen-snapshots.mjs`: WebContainer's `spawn` only works inside the full app (not a stripped page), so it drives the built playground in headless Chromium (Playwright) via `?__generate&template=<name>`, which runs a fresh install and exposes the gzip export on `window.__pikaSnapshot`. The deploy workflow runs it after `pnpm playground:build` (non-fatal). `.bin` files are build artifacts, not committed.
