...
- Do **not** try to speed up boot by pre-installing dependencies on the host and mounting them as a static snapshot: WebContainer disables native `.node` addon loading and runs its own WASM builds of native-dependent packages (rollup, esbuild) resolved only by an *in-container* `npm install` via StackBlitz's proxy registry. A host-built `node_modules` ships incompatible native binaries and fails at dev-server startup (`ERR_DLOPEN_DISABLED` for rollup, esbuild "service was stopped"). Any dependency-cache work must use WebContainer's own `export()` output, not a host install.
- `PreviewPanel.vue` reloads the preview iframe once shortly after `server-ready`: the dev server becomes ready before PikaCSS has written `pika.gen.css`, so the first paint is unstyled and Vite's CSS HMR does not retroactively style it.
