...
- Marketing/example copy that shows a literal `pika()` call must keep the `pika(` token out of the source, or the transform's regex scanner (`stripLiteral` + `\bpika\(`) rewrites the bare `pika()` in JSX/template **text** to `''`. In JSX wrap it in a JS string: `{'pika()'}`. In a **Vue template** use HTML entities `pika&#40;&#41;` — a mustache like `{{ 'pika()' }}` does *not* work: the transform still mangles it and Vue then fails to parse the interpolation.
